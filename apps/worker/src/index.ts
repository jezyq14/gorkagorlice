...
async function runLuckyNumbersJob() {
    ...
    try {
        ...
        if (luckyData) {
            logger.info(`Successfully fetched! Date: ${luckyData.dateString}, Numbers: ${luckyData.numbers.join(', ')}`);
            ...
            logger.warn('Scraper hasn\'t found lucky numbers! School website structure may have been changed.');
        }
    } catch (error) {
